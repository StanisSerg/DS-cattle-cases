...
- столбец «Пик прод-ть день» опускается (решение пилота); «Продуктивность» переносится;
...
def main() -> int:
    ...
    out_path = args.case_dir / "raw" / "excel" / "Excel-MD" / f"{m.group(1)}_dairyplan_produktivnost.md"
    ...
    for r in ws.iter_rows(min_row=6, values_only=True):
        ...
        rows.append((
            ...
            f"{float(prod):.1f}" if isinstance(prod, (int, float)) else "",
            fixed_event,
        ))
