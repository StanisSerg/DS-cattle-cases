#!/usr/bin/env python3
"""Парный анализ до/после корректировки рациона 10.06.2026 (WP-118 Ф3).

Вопрос: рост продуктивности после 10.06.2026 — эффект рациона или композиционный
эффект отёлов мая (аргумент зоотехника)?

Метод:
1. Когорта: коровы, доившиеся на 10.06.2026 (срез в день корректировки рациона) и найденные
...
_T95 = {1: 12.706, 2: 4.303, 3: 3.182, 4: 2.776, 5: 2.571, 6: 2.447, 7: 2.365,
        ...
        18: 2.101, 20: 2.086, 25: 2.060, 30: 2.042}


def _ci(deltas):
    """95% ДИ среднего парного прироста: mean ± t(0.975, n-1) * sd/√n."""
    n = len(deltas)
    if n < 2:
        return {"sd": None, "ci": None}
    mean = sum(deltas) / n
    sd = (sum((x - mean) ** 2 for x in deltas) / (n - 1)) ** 0.5
    df = n - 1
    t = _T95.get(df, _T95[max(k for k in _T95 if k <= df)] if df < 30 else 1.96)
    if df > 30:
        t = 1.96
    half = t * sd / n ** 0.5
    return {"sd": sd, "ci": (mean - half, mean + half)}


# --- корректировка рациона 10.06.2026 ---
...
# --- декомпозиция «состав vs коровы» для диапазона корректировки рациона 10.06.2026 и контроля 2025 ---
...
def fmt(stats):
    ...
    for name, s in stats.items():
        if s is None:
            ...
            continue
        ci = f"{s['ci'][0]:+.1f}…{s['ci'][1]:+.1f}" if s.get("ci") else "—"
        rows.append(
            f"| {name} | {s['n']} | {s['avg0']:.1f} | {s['avg1']:.1f} "
            f"| {s['delta']:+.1f} | {ci} | {s['up']} ({s['up']/s['n']*100:.0f}%) | {s['down']} ({s['down']/s['n']*100:.0f}%) |"
        )
    return rows
...
lines = [
    ...
    "---",
    "",
    "# Влияние корректировки рациона 10.06.2026: эффект рациона vs эффект отёлов",
    "",
    "## Резюме простым языком (для зоотехника)",
    "",
    "**Спорный вопрос:** молоко пошло вверх после 10 июня потому, что мы поменяли кормление —",
    "или просто в мае отелилось много коров, а свежие коровы всегда дают больше?",
    "",
    "**Как проверяли.** В программе записаны все коровы поимённо, два раза в месяц, полтора года.",
    "Мы взяли коров, которые **уже доились 10 июня** — до смены рациона. Никаких майских новотёлок",
    "среди них нет. И посмотрели, что стало с их надоями через месяц.",
    "",
    "**Что получилось.**",
    "",
    "*1. Старые коровы сами прибавили.* 741 корова, доившаяся до 10 июня, за месяц прибавила",
    "в среднем **+2,4 кг на голову**. Две из трёх коров выросли. Новые коровы в этот расчёт",
    "вообще не входили — поэтому отёлы не могут быть объяснением.",
    "",
    "*2. Год назад в это же время было наоборот.* В июне–июле 2025 те же по положению коровы",
    "**теряли** молоко (−0,7 кг за месяц, жара). Этим летом в ту же жару — прибавляли.",
    "Значит, дело не в сезоне.",
    "",
    "*3. Коровы на излёте лактации пошли вверх — а они обязаны идти вниз.* Корова после 150 дней",
    "доения по природе снижает надой примерно на килограмм в месяц. Год назад так и было (−1,3 кг).",
    "После смены кормления эти коровы **прибавили** +0,7 кг. Природой это объяснить нельзя —",
    "кормлением можно.",
    "",
    "*4. Отёлы посчитаны в килограммах.* Общий рост стада за месяц — +3,7 кг. Из них от свежих",
    "отелившихся коров — **+0,7 кг, то есть 18%**. Остальные 82% дали старые коровы и уход слабых.",
    "«Рост за счёт отёлов» цифрами не подтверждается.",
    "",
    "*5. Самые маленькие по лактации выросли сильнее всех.* Группа 0–10 дней (им двухразовое",
    "кормление): **+11 кг** за месяц. Дальше эффект убывает по группам ровно в порядке удаления",
    "от мишени корректировки. Так работает прицельная мера, а не общий фон.",
    "",
    "*6. Проверили, не совпадение ли это.* Прогнали тот же расчёт для 34 фиктивных дат за полтора",
    "года. Ни одна не дала ничего похожего на +2,4 кг (лучшая — +1,2). Эффект привязан именно",
    "к 10 июня.",
    "",
    "**Вывод:** рост продуктивности летом 2026 — следствие изменений в кормлении 10 июня (рацион",
    "для свежих коров + работа с кормосмесью: контроль остатков, контроль сухого вещества,",
    "двухразовое кормление новотельных). Отёлы мая добавили меньше пятой части прироста.",
    "",
    "**Что мы честно не утверждаем:** данные не разделяют, что сильнее сработало — рацион для",
    "группы 0–100 или технологические меры по всей кормосмеси (коровы после 100 дней тоже немного",
    "выросли). Сработал весь пакет 10 июня.",
    "",
    "---",
    "",
    "## Технические разделы",
    ...
    "",
    "**Метод:** парный анализ до/после по одним и тем же коровам. В когорту входят только",
    ...
    "",
    "## Корректировка рациона 10.06.2026: 10.06 → 10.07 (30 дней)",
    "",
    ...
    *fmt(stats26),
    ...
    "",
    "| Когорта (DIM на 01.06) | N | Надой 01.06 | Надой 01.07 | Δ средн. | 95% ДИ | Выросли | Упали |",
    "|---|---|---|---|---|---|---|---|",
    *fmt(stats25),
    "",
    "## Diff-in-diff: лечённая группа (0–100) против контроля внутри стада (100+)",
    "",
    "Корректировка рациона 10.06.2026 была точечной: рацион для группы 0–100 дней + технологические",
...
lines += [
    ...
    "|---|---|---|---|",
    f"| 2026 (корректировка 10.06) | {dt26:+.1f} | {dc26:+.1f} | **{dt26 - dc26:+.1f}** |",
    f"| 2025 (без корректировки) | {dt25:+.1f} | {dc25:+.1f} | {dt25 - dc25:+.1f} |",
    "",
    f"**Двойное различие: ({dt26 - dc26:+.1f}) − ({dt25 - dc25:+.1f}) = {(dt26 - dc26) - (dt25 - dc25):+.1f} кг** — ",
    "чистый эффект корректировки рациона на целевую группу поверх сезона и фона.",
    "",
]
...
lines += [
    "## Плацебо-тест: те же расчёты с фиктивными датами корректировки",
    "",
...
lines += [
    ...
    f"а рост коров, доившихся ещё до корректировки, — {dec26['a']:+.1f} кг.",
    "",
    "## График",
    "",
...
for d1, y, m, rate, n in growth:
    ...
    mark = " **← после корректировки**" if d1 > date(2026, 6, 10) else ""
...
ax.set_title("Одни и те же коровы до и после корректировки рациона 10.06.2026 (10.06→10.07.2026 vs 01.06→01.07.2025)")
ax.legend()
...
ax.text(date(2026, 6, 12), ax.get_ylim()[1] - 0.5, "корректировка рациона 10.06.2026", fontsize=9, color="#b03a3a")
...
# --- график плацебо: прирост всей когорты по всем диапазонам ---
fig, ax = plt.subplots(figsize=(12, 5))
px = [d0 for d0, _, da, _ in placebo_pre]
py = [da for _, _, da, _ in placebo_pre]
real_da = stats26["ИТОГО"]["delta"]
bars = ax.bar(px, py, width=20, color="#8D99AE", label="фиктивные диапазоны (30 дней)")
ax.bar(date(2026, 6, 10), real_da, width=20, color="#FF6B6B", zorder=5,
       label=f"реальная дата 10.06.2026 ({real_da:+.1f} кг)")
ax.annotate(f"{real_da:+.1f}", (date(2026, 6, 10), real_da), xytext=(0, 4),
            textcoords="offset points", ha="center", fontsize=11, fontweight="bold", color="#2B2D42")
best = max(placebo_pre, key=lambda t: t[2])
ax.annotate(f"{best[2]:+.1f} — лучшее плацебо", (best[0], best[2]), xytext=(0, 4),
            textcoords="offset points", ha="center", fontsize=9, color="#2B2D42")
ax.axhline(0, color="#2B2D42", lw=0.8)
ax.axvline(date(2026, 6, 10), color="#FF6B6B", ls="--", lw=1.2, alpha=0.6)
ax.set_title("Плацебо-тест: прирост одних и тех же коров за 30 дней по всем диапазонам 2025–2026")
ax.set_ylabel("Прирост когорты за 30 дней, кг")
ax.legend()
ax.grid(alpha=0.3, axis="y")
...
fig.autofmt_xdate()
fig.tight_layout()
fig.savefig(CASE / "charts" / "placebo_intervention.png", dpi=150)
plt.close(fig)
